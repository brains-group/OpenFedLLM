...
def get_proxy_dict(fed_args, global_dict):
    ...
    if fed_args.fed_alg in ["fedadagrad", "fedyogi", "fedadam"]:
        ...
        for key in global_dict.keys():
            proxy_dict[key] = torch.zeros_like(global_dict[key]).to(device="cuda")
            opt_proxy_dict[key] = (
                torch.ones_like(global_dict[key]).to(device="cuda") * fed_args.fedopt_tau**2
            )
...
def get_auxiliary_dict(fed_args, global_dict):
    ...
    if fed_args.fed_alg in ["scaffold"]:
        ...
        for key in global_dict.keys():
            global_auxiliary[key] = torch.zeros_like(global_dict[key]).to(device="cuda")
        auxiliary_model_list = [
            copy.deepcopy(global_auxiliary) for _ in range(fed_args.num_clients)
        ]  # c_i in SCAFFOLD
        auxiliary_delta_dict = [
            copy.deepcopy(global_auxiliary) for _ in range(fed_args.num_clients)
        ]  # delta c_i in SCAFFOLD
