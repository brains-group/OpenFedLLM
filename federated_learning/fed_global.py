    ...
    if (fed_args.fed_alg).startswith("local"):
        ...
        if fed_args.num_clients < fed_args.sample_clients:
            ...
            clients_this_round = sorted(
                random.sample(range(fed_args.num_clients), fed_args.sample_clients)
            )
    ...
    if fed_args.fed_alg == "scaffold":
        ...
        for key, param in opt_proxy_dict.items():
            ...
            opt_proxy_dict[key] = param - (
                1 - fed_args.fedopt_beta2
            ) * delta_square * torch.sign(param - delta_square)
            global_dict[key] += fed_args.fedopt_eta * torch.div(
                proxy_dict[key], torch.sqrt(opt_proxy_dict[key]) + fed_args.fedopt_tau
            )
        ...
        for key, param in opt_proxy_dict.items():
            ...
            opt_proxy_dict[key] = fed_args.fedopt_beta2 * param + (
                1 - fed_args.fedopt_beta2
            ) * torch.square(proxy_dict[key])
            global_dict[key] += fed_args.fedopt_eta * torch.div(
                proxy_dict[key], torch.sqrt(opt_proxy_dict[key]) + fed_args.fedopt_tau
            )
            ...
            global_dict[key] = sum(
                [
                    (
                        local_dict_list[client][key]
                        + gaussian_noise(
                            local_dict_list[client][key].shape,
                            fed_args,
                            script_args,
                            local_dict_list[client][key].device,
                        )
                    )
                    * sample_num_list[client]
                    / sample_this_round
