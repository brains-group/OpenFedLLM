...
class FedArguments:
    ...
    checkpoint_step: Optional[int] = field(
        default=50, metadata={"help": "rounds to save checkpoints"}
    )
    ...
    fedopt_beta1: Optional[float] = field(
        default=0.5, metadata={"help": "the beta1 parameter of FedYogi and FedAdam"}
    )
...
class ScriptArguments:
    finetune_method: Optional[str] = field(
        default="dpo",
        metadata={"help": "the finetuning method name (sft, dpo, or kto)"},
    )
    redistribute_dataset: Optional[bool] = field(
        default=False,
        metadata={
            "help": "whether to randomly redistribute the dataset points across clients"
        },
    )
    ...
    logging_steps: Optional[int] = field(
        default=100, metadata={"help": "the number of logging steps"}
    )
    use_auth_token: Optional[bool] = field(
        default=False, metadata={"help": "Use HF auth token to access the model"}
    )  # token and use_auth_token cannot be used together
    num_train_epochs: Optional[int] = field(
        default=3, metadata={"help": "the number of training epochs"}
    )
    max_steps: Optional[int] = field(
        default=10, metadata={"help": "the number of training steps"}
    )
    ...
    dpo_beta: Optional[float] = field(
        default=0.1, metadata={"help": "the beta parameter of DPO"}
    )
    kto_beta: Optional[float] = field(
        default=0.1, metadata={"help": "the beta parameter of KTO"}
    )
    dataset_sample: Optional[int] = field(
        default=20000,
    ...
    multi_turn_task: Optional[bool] = field(
        default=False, metadata={"help": "Whether to train multi-turn task"}
    )
    dynamic_local_step: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to user dynamic local step(for chatbotPA tasks)"},
    )
    # for differential privacy
    dp_max_grad_norm: Optional[float] = field(
        default=1.0, metadata={"help": "the max grad norm for differential privacy"}
    )
    dp_delta: Optional[float] = field(
        default=1e-4, metadata={"help": "the delta for differential privacy"}
    )
    dp_epsilon: Optional[float] = field(
        default=1.0, metadata={"help": "the delta for differential privacy"}
    )
    dp_sigma: Optional[float] = field(
        default=None,
        metadata={
...
if script_args.use_peft:
    ...
    if script_args.model_name_or_path.find("Qwen") >= 0:
...
# def get_dpo_training_args(script_args, new_lr, local_step):
...
def save_config(script_args, fed_args):
    ...
    output_dir = f"{script_args.output_dir}/{dataset_name_split}_{script_args.dataset_sample}_{fed_args.fed_alg}_{script_args.finetune_method}{'_dist' if script_args.redistribute_dataset else ''}_c{fed_args.num_clients}s{fed_args.sample_clients}_i{script_args.max_steps}_b{script_args.batch_size}a{script_args.gradient_accumulation_steps}_l{script_args.seq_length}_r{script_args.peft_lora_r}a{script_args.peft_lora_alpha}_{now_time}"
    while True:
        if not os.path.exists(output_dir):
            ...
            output_dir = f"{script_args.output_dir}/{dataset_name_split}_{fed_args.fed_alg}_{script_args.finetune_method}{'_dist' if script_args.redistribute_dataset else ''}_c{fed_args.num_clients}s{fed_args.sample_clients}_i{script_args.max_steps}_b{script_args.batch_size}a{script_args.gradient_accumulation_steps}_l{script_args.seq_length}_{now_time}"
