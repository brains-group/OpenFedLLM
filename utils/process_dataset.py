...
def process_kto_dataset(dataset_name, dataset, template_name, dataset_sample):
    if dataset_name in ["Anthropic/hh-rlhf"]:
        dataset = dataset.map(
            partial(split_hh, template_name=template_name), load_from_cache_file=False
        )
    ...
    dataset = Dataset.from_dict(
        {
            "prompt": dataset["prompt"] + dataset["prompt"],
            "completion": dataset["chosen"] + dataset["rejected"],
            "label": ([True] * dataset.num_rows) + ([False] * dataset.num_rows),
        }
    )
