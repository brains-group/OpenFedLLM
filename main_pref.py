...
if script_args.use_peft == True:
    ...
    # e.g. DPO and KTO need a reference model to compute the discrepancy loss
...
for round in tqdm(range(fed_args.num_rounds)):
    ...
    for client in range(fed_args.num_clients):
        ...
        training_args = (
            get_kto_training_args(script_args, new_lr, local_step, sub_dataset)
            if script_args.finetune_method == "kto"
            else get_training_args(script_args, new_lr, local_step)
        )

        # ===== Train local model on the client side =====
        trainer = (
            get_fed_local_kto_trainer
            if script_args.finetune_method == "kto"
            else get_fed_local_dpo_trainer
        )(
            script_args,
            fed_args,
            model,
