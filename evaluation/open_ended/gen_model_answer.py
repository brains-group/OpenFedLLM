...
if args.use_vllm and args.lora_path is not None:
    raise ValueError(
        "Cannot use both VLLM and LORA, need to merge the lora and then use VLLM"
    )
...
if args.bench_name == "alpaca":
    ...
    eval_set = datasets.load_dataset("json", data_files="data/vicuna/question.jsonl")[
        "train"
    ]
    ...
    def rename(example):
        ...
        example["question_body"] = example["turns"][0]
        return example

    eval_set = eval_set.map(rename)
    max_new_tokens = 2048
elif args.bench_name == "advbench":
    eval_set = datasets.load_dataset("csv", data_files="data/advbench/advbench.csv")[
        "train"
    ]
...
if args.use_vllm:
    ...
    if args.bench_name == "advbench":
        input_list = [
            template.format(example["instruction"] + ".", "", "")[:-1]
            for example in eval_set
        ]
    else:
        input_list = [
            template.format(example["instruction"], "", "")[:-1] for example in eval_set
        ]  # TODO: use fastchat conversation
    input_list = input_list[existing_len:]
    print(f">> Example input: {input_list[0]}")
    sampling_params = SamplingParams(
        temperature=0.7, top_p=1.0, max_tokens=max_new_tokens
    )
    ...
    for i, example in tqdm(enumerate(eval_set)):
        ...
        if args.bench_name == "advbench":
            ...
            instruction = template.format(example["instruction"], "", "")[
                :-1
            ]  # TODO: use fastchat conversation
        ...
        example["decoding_method"] = "top_p_sampling"
        example["scores"] = {}
        ...
        save_jsonl(result_list, result_path + "l")
        with open(result_path, "w") as f:
            json.dump(result_list, f, indent=4)
