...
# MMLU
sys.argv = ["", "--base_model_path", dpo_model_path]
subprocess.run(
    f'conda run --no-capture-output -n deepeval python ./MMLU_evaluation.py {" ".join(sys.argv)}',
    shell=True,
)
sys.argv = ["", "--base_model_path", kto_model_path]
subprocess.run(
    f'conda run --no-capture-output -n deepeval python ./MMLU_evaluation.py {" ".join(sys.argv)}',
    shell=True,
)
sys.argv = ["", "--base_model_path", kto_dist_model_path]
subprocess.run(
    f'conda run --no-capture-output -n deepeval python ./MMLU_evaluation.py {" ".join(sys.argv)}',
    shell=True,
)
